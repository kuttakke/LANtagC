...
use tabled::{settings::Format, settings::object::Columns};
...
fn parse_data(data: &serde_json::Value) -> Result<HashMap<String, String>, Box<dyn std::error::Error>> {
...
fn get_regex_title(title: &str) -> String {
    if let Ok(regex) = Regex::new(r"(\[.*?\])\s*(.*?)\s*(?:#.*?)?\s*(?:\([^)]*\))?\s*(?:｜|︱)?\s*(?:\([^)]*\))?\s*(\[|$)") {
...
async fn search_from_eh(title: &str) -> Vec<GL> {
    ...
    let url = format!("{}{}", url, form_urlencoded::byte_serialize(title.as_bytes()).collect::<String>());
    ...
    for tr in trs{
        ...
        let type_ = tr.select(&Selector::parse("td:nth-child(1) div").unwrap()).next().unwrap().text().collect::<String>();
        let datetime = tr.select(&Selector::parse("td:nth-child(2) div:nth-child(3) div:nth-child(1)").unwrap()).next().unwrap().text().collect::<String>();
        let url = tr.select(&Selector::parse("td:nth-child(3) a").unwrap()).next().unwrap().value().attr("href").unwrap().to_string();
        let title = tr.select(&Selector::parse("td:nth-child(3) a div:nth-child(1)").unwrap()).next().unwrap().text().collect::<String>();
        let pages = tr.select(&Selector::parse("td:nth-child(4) div:nth-child(2)").unwrap()).next().unwrap().text().collect::<String>();
        let tagnods_selector = Selector::parse("td:nth-child(3) a div:nth-child(2) div.gt").unwrap();
...
fn print_and_get_index(gls: &Vec<GL>, archive: &Archive) -> i32{
    ...
    for (i, gl) in gls.iter().enumerate(){
        ...
        builder.push_record([&(i as i32+1).to_string(), &gl.title, &format!("{:.1}%", similarity).to_string(), &page_flag, &gl.datetime]);
    }
    let mut table = builder.build();
    table.with(Style::rounded())
    .modify(Columns::single(0), Format::content(|s| s.cyan().to_string()))
    .modify(Columns::single(1), Format::content(|s| s.magenta().to_string()))
    .modify(Columns::single(2), Format::content(|s| s.green().to_string()))
    .modify(Columns::single(3), Format::content(|s| s.blue().to_string()))
    .modify(Columns::single(4), Format::content(|s| s.yellow().to_string()));
    println!("{} - {}", &archive.title.bright_blue(), &archive.pagecount.bright_blue());
    println!("{}", table);
    // get user input
    let mut input = String::new();
...
async fn change_tags_to_lanraragi(archive: &Archive, tags: &str){
    let url = format!("{}/api/archives/{}/metadata", endpoint(), &archive.arcid);
    ...
    let resp = reqwest::Client::new().put(url).form(&form_data).send().await.unwrap();
...
async fn get_tags_from_eh_gl(gl: &GL, cn_tags: &HashMap<String, String>) -> String {
    ...
    tags_str.push_str(&format!("source:{}", &gl.url.trim_start_matches("https://")));
...
fn write_no_handle(archive: &Archive){
    let mut file = OpenOptions::new()
    .create(true)
    .append(true)
    .open("no_handle.txt").unwrap();

    let mut content = serde_json::to_string(archive).unwrap();
    content.push_str("\n");
...
async fn main() {
    ...
    let all_archive: Vec<Archive> = fetch(&format!("{}/api/archives", endpoint())).await.unwrap();
    println!("获取最新的cn标签...");
    ...
    for archive in all_archive.iter(){
        ...
        if index > 0 && index <= gls.len() as i32 {
            ...
            run_count += 1;
            println!("已处理 {}/{}  {:.1}%", run_count.green(), all_archive.len().cyan(), (run_count as f32 / all_archive.len() as f32 * 100.0).bright_green());
        }else {
            // no_handle_array.push(archive);
            write_no_handle(archive);
            run_count += 1;
